...
def get_full_ride_data(id_ride: int):
...
@cache.memoize(timeout=86400)
def get_track_for_id(id_ride: int):
    db = get_db()
    tracks = Table("tracks_enhanced_v1")
    query = (
...
@cache.memoize(timeout=86400)
def get_thumbnails_for_id(id_ride: int):
...
@cache.memoize(timeout=86400)
def get_rides_in_timeframe(timeframe: int | str):
    db = get_db()
    main = Table("main")
    tracks = Table("tracks_enhanced_v1")
    track_data = Table("tracks_v1_overview")
...
def get_summary_data(timeframe: int | str, current_year: int, curr_month: int):
    ...
    summary_data = [
        ("Total Distance [km]", str(round(rides.distance.sum(), 2))),
        ("Total Ride time ", str(rides.ride_time.sum()).split(".")[0]),
        ("Number of rides", str(len(rides))),
        ("Avg. distance [km]", str(round(rides.distance.mean(), 2))),
        (
            "Avg. ride duration",
            str(rides.ride_time.mean()).split(".")[0].replace("0 days ", ""),
        ),
    ]
